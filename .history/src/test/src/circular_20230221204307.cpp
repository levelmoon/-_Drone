...
{
	position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/mavros/local_position/pose", 100, &planner::pos_cb,this);
...
int main(int argc, char **argv)
{
	...
	planner planner_node(&nh);//init some param and then start the controller 
   	...
   	// nh.param<float>("desire_z", planner_node.desire_z, 1.0);
   	// nh.param<float>("desire_Radius", planner_node.desire_Radius, 1.0);
