...
mavros_msgs::RCIn get_rc_in;//get the data of rc channels
int switch_yaw_xyz[4];// this used to store the number of rc channels
void get_rc_in_cb(const mavros_msgs::RCIn::ConstPtr& msg)
{
	get_rc_in = *msg;
	ROS_INFO_STREAM("ALL_in: "<<get_rc_in);
	
	for(int i =0 ; i<4; i++)
	{
		switch_yaw_xyz[i]=get_rc_in.channels[i];
	}
...
mavros_msgs::RCOut get_rc_out;
void get_rc_out_cb(const mavros_msgs::RCOut::ConstPtr& msg)
{
	get_rc_out = *msg;
	ROS_INFO_STREAM("ALL_out: "<<get_rc_out);
}
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber get_rc_in_sub = nh.subscribe<mavros_msgs::RCIn>("/mavros/rc/in",10,get_rc_in_cb);
